...
@Injectable()
export class QTestSpecService implements IExistService {
  private baseUrl = '/api/qualificationTestSpecs';

  private eliteFields = '_id oid name ver';
  private profileFields = '_id oid name ver';
  ...
  private log(message: string) {
    this.messageService.add(`QTestSpecService: ${message}`);
  }
  ...
  getQTest(id: string): Observable<IQTest> {
    ...
    return this.http.get<IQTest>(url)
      .pipe(
        tap(_ => this.log('fetched QTests id=${id}')),
        catchError(this.handleError<IQTest>('getQTests'))
      );
  }
