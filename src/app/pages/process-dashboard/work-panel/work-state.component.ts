...
@Component({
  selector: 'process-work-state',
  template: `
    <div echarts [options]="options" class="echart" (chartClick)="onChartClick($event)"></div>
  `,
})
export class WorkStateComponent implements AfterViewInit, OnDestroy {
  _hs: IHierarchyScope;
  @Input() 
  ...
  init(): void {
    ...
    this.wrService.aggr('state', this._hs, this._startTime, this._endTime).subscribe(items => {
