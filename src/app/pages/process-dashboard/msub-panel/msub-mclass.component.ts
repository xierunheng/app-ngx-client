...
@Component({
  selector: 'mes-msub-mclass',
  template: `
    <div echarts [options]="options" class="echart"  (chartClick)="onChartClick($event)"></div>
  `,
  styles: []
})
export class MsubMclassComponent implements AfterViewInit, OnDestroy {
  _hs: IHierarchyScope;
  @Input()
  ...
  @Output() mclassChange: EventEmitter<string> = new EventEmitter<string>();
  ...
  init(): void {
    if (this._hs && this._startTime && this._endTime) {
      ...
      // this.mslService.aggrClass(this._hs, this._startTime, this._endTime)
      .subscribe(items => {
        console.log(items);
            ...
            xAxis: [
              {
                ...
                data: items.map(item => item.name[0].oid),
                axisTick: {
                  alignWithLabel: true,
                },
                axisLine: {
  ...
  onChartClick(event){
    console.log(event);
    this.mclassChange.emit(event.name);
  }
