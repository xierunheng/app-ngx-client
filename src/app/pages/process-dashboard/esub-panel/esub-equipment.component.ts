...
@Component({
  selector: 'mes-esub-equipment',
  template: `
    <div echarts [options]="options" class="echart" (chartClick)="onChartClick($event)"></div>
  `,
  styles: []
})
export class EsubEquipmentComponent implements AfterViewInit, OnDestroy {
  _hs: IHierarchyScope;
  @Input() 
  ...
  init(): void {
    if(this._hs && this._startTime && this._endTime && this._eclass) {
    this.esubService.aggr('equipment', this._hs, this._startTime, this._endTime, {'eclass.oid': this._eclass}).subscribe(items => {
      console.log(items);
