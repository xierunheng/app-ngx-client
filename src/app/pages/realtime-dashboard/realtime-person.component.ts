...
@Component({
  selector: 'mes-realtime-person',
  template: `
    <div echarts [options]="options" class="echart"></div>
  `,
})
export class RealtimePersonComponent implements OnDestroy {
  private alive = true;
  ...
  init(hs: IHierarchyScope): void {
    ...
    this.psubService.aggr('person.name', hs, startTime, endTime).subscribe(items => {
  ...
  startReceivingLiveData() {
    ...
    this.intervalSubscription = interval(60 * 1000)
      .pipe(
        ...
        switchMap(() => this.psubService.aggr('person.name', this._hs, startTime, endTime))
      )
      .subscribe(data => {
